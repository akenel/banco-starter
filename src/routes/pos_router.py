...
# Starter/single-host override for the BROWSER login leg. When POS_KC_PUBLIC_URL is set
# (e.g. a local `docker compose` stand-up where Keycloak is on http://localhost:8080),
# the login page hits THAT Keycloak origin directly. Unset (prod) → empty string → the
# page keeps its existing same-origin/host logic. Backward-compatible by construction.
templates.env.globals["pos_kc_public_url"] = os.environ.get("POS_KC_PUBLIC_URL", "")
