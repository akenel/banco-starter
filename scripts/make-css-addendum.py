...
def main():
    ...
    # ── GRADIENTS ARE ORDER-DEPENDENT AND THAT BIT ME ────────────────────────────────
    # Tailwind's `from-*` sets --tw-gradient-stops to TWO stops; `via-*` then rewrites it
    # with three. In the vendored file they sit in that order, so via wins. Put a lone
    # `from-*` in a stylesheet that loads AFTER tailwind.css and it clobbers the via that
    # already ran: on prod the UAT login computed to
    #     linear-gradient(to right bottom, rgb(14,116,144), rgba(14,116,144,0))
    # — cyan fading to nothing, no teal, no slate. A gradient, which is all the first
    # version of prove-classes-exist.js asked for, and not the one anybody designed.
    #
    # So every palette gets its whole triple re-emitted here, in order, and the addendum
    # ends up internally consistent no matter what the vendored file did.
    # Bucketed by ROLE, not by palette. Deduping across palettes is what broke it the
    # first time: `via-teal-800` was emitted once for the emerald set, skipped for the
    # cyan set, and `from-cyan-700` therefore landed AFTER it and clobbered the stops all
    # over again. Every from- goes out before every via-, and every via- before every to-.
    grad_roles = [[], [], []]
    for f in sorted(glob.glob(os.path.join(TPL, '*.html'))):
        txt = open(f).read()
        for m in re.finditer(r"{%-?\s*set\s+\w+\s*=\s*'([^']*)'", txt):
            toks = m.group(1).split()
            if any(tk.startswith('from-') for tk in toks) and any(tk.startswith('via-') for tk in toks):
                for tk in toks:
                    for role, bucket in (('from-', 0), ('via-', 1), ('to-', 2)):
                        if tk.startswith(role):
                            body = decls(tk, pal)
                            if body:
                                line = f'.{esc(tk)} {{ {body} }}\n'
                                if line not in grad_roles[bucket]:
                                    grad_roles[bucket].append(line)
    ...
    open(OUT, 'w').write(head + ''.join(r for _, r in rules)
                         + (grad_head + ''.join(sum(grad_roles, [])) if any(grad_roles) else ''))
