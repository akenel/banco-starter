...
| **the conditions a sheet depends on** | `setup: { needs: [...] }` in the SHEET object. The shell turns it into **step 0.2**, and the three rig selectors (folio · gun · orientation) ride in the copied report — or it prints **DEVICE STATE NOT RECORDED**. Added 2026-09-04 after two runs were invalidated for having the folio keyboard attached: the testers reached for the folio instead of the pad, and the pad was the subject of the test. ⚠️ **Corrected 2026-09-17** — this used to read *"the on-screen pad never appeared"*, which is FALSE and sent a doc out saying the app disables itself when the folio is on. It does not: the only gate in `pos-keypad.js` is `isTouch && !isPhone`, there is no hardware-keyboard detection in it, and Angel confirmed on the tablet that both keyboards work together. The rig selectors still earn their place — what they catch is which keyboard the human used. A PASS had to be thrown out and the date fields passed for a day while unusable on the tablet alone. |
