...
- 2026-07-28 — The fix for the `ipp-usb` mess was to **drop it entirely**: `printer-driver-ptouch` (Debian, open source, lists the QL-820NWB as *recommended*) + CUPS's own `usb://` backend, with ipp-usb blacklisted via a quirks file. Prints in ~4 s instead of 25–60 s, and gives print density, auto-cut and continuous-tape lengths the generic `everywhere` driver never had.
